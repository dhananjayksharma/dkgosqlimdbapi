...
type MySQLDBStoreAccess interface {
	GetMovieList(ctx context.Context, movieData *[]response.MovieResponse) error
	...
	ListPerson(ctx context.Context, user *[]response.PersonResponse) error
	// ListPersonByName(ctx context.Context, user *[]response.PersonResponse, queryParams request.QueryMembersInputRequest) error
}
...
// ListMovieDetail
func (ms *mySQLDBStore) ListMovieDetail(ctx context.Context, merchant *[]response.MovieDetailsResponse, queryParams request.QueryMembersInputRequest) error {

	result := ms.db.Debug().WithContext(ctx).Model(&response.MovieDetailsResponse{}).Select("person.name, person.email, person.mobile, person.is_active, person.created_at, movies.name as MovieName").Joins("left join merchants on movies.code = person.fk_code").Where("fk_code=?", queryParams.Code).Limit(queryParams.Limit).Offset(queryParams.Skip).Scan(&merchant)
...
// GetMovieList
func (ms *mySQLDBStore) GetMovieList(ctx context.Context, movieData *[]response.MovieResponse) error {
	result := ms.db.WithContext(ctx).Model(&response.MovieResponse{}).Select("moviecode,  name, is_active, created_at, updated_at").Find(&movieData)
	err := result.Error
	if err != nil {
		return &util.InternalServer{ErrMessage: err.Error()}
	}
...
func (ms *mySQLDBStore) ListMovieByCode(ctx context.Context, movie *entities.Movie, code string) error {
	result := ms.db.WithContext(ctx).Model(&response.MovieResponse{}).Select("moviecode,  name, is_active, created_at, updated_at").Find(&movie).Where("moviecode=?", code)
	err := result.Error
	if err != nil {
		return &util.InternalServer{ErrMessage: err.Error()}
	}
